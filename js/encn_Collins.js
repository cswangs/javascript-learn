...
class encn_Collins {
    ...
    async findCollins(word) {
        ...
        try {
            ...
            let collins = getCollins($);
            let youdao = collins.length ? [] : getYoudao($); //downgrade to Youdao Concise English-Chinese Dictionary if not Collins.
            // let collins = []
            // let youdao = []
            let ydtrans = collins.length || youdao.length ? [] : getYDTrans($); //downgrade to Youdao Translation (if any) to the end.
        ...
        function getCollins($) {
            ...
            let defNodes = $('#collinsResult .ol li')
            if (!defNodes || !defNodes.length) return notes;

            //get headword and phonetic
            let expression = T($('#collinsResult h4 .title')); //headword
            let reading = T($('#collinsResult h4 .phonetic')); // phonetic
            //get exam cert
            let extra_cet = '';
            let cets = T($('#collinsResult h4 .rank'));
            if (cets) {
            ...
            //get Collins star
            let extra_star = '';
            let starNode = $('#collinsResult h4 .star');
            let star = starNode ? starNode.attr('class').split(' ')[1].substring(4, 5) : '';
            ...
            for (const defNode of defNodes) {
                let definition = '';
                let tranNode = $(defNode, '.collinsMajorTrans p');
                if (!tranNode) continue;
                let posNodes = $(tranNode, '.additional');
                let pos = '';
                ...
                let exampleNodes = $(defNode, '.exampleLists');
                if (exampleNodes && exampleNodes.length > 0 && maxexample > 0) {
                    definition += '<ul class="sents">';
                    exampleNodes.each(function (index, example) {
                        if (index > maxexample - 1) return; // to control only n example sentences defined in option.
                        let chn_sent = T($(example, 'p+p'));
                        let eng_sent = T($(example, 'p')) ? T($(example, 'p')).replace(RegExp(expression, 'gi'), '<b>$&</b>') : ''; //surround expression with <b> in eng_example.
        ...
        function getYoudao($) {
            ...
            let defNodes = $('#phrsListTab .trans-container ul li');
            if (!defNodes || !defNodes.length) return notes;

            //get headword and phonetic
            let expression = T($('#phrsListTab .wordbook-js .keyword')); //headword
            let reading = '';
            let readings = $('#phrsListTab .wordbook-js .pronounce');
            if (readings) {
        ...
        function getYDTrans($) {
            let notes = [];
            //get Youdao EC data: check data availability
            let transNode = $($('#ydTrans .trans-container p')[1]);
...
new encn_Collins("hello").findTerm("sounds").then(x => console.log(JSON.stringify(x)))